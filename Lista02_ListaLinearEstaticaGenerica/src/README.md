...
- Exception Test: Guarantees that IndexOutOfBoundsException is thrown when attempting to access invalid positions or an empty list.
...
- Teste de Exceção: Garante que a exceção IndexOutOfBoundsException é lançada ao tentar acessar posições inválidas ou operar em uma lista vazia.
